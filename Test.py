...
from FS_MS2WT import FEM2Transformer
...
parser.add_argument("--testset_num", default=3, type=int, help='total number of testset')
...
HR_HSI, HR_MSI = prepare_data(opt.data_path, file_list, opt.testset_num)

dataset = cave_dataset(opt, HR_HSI, HR_MSI, istrain=False)
loader_train = tud.DataLoader(dataset, batch_size=opt.batch_size)

checkpoint_path = "./Checkpoint/f4/FS_MS2WT/model_0295_state_dict.pth"
checkpoint = torch.load(checkpoint_path, map_location=device)
if isinstance(checkpoint, torch.nn.Module):
    model = checkpoint
else:
    model = FEM2Transformer(4, 3, 31)
    model.load_state_dict(checkpoint)
model = model.eval().to(device)

result_dir = './Result/f4/FEM2Transformer/'
...
for j, (LR, RGB, HR) in enumerate(loader_train):
    with torch.no_grad():
        ...
        scipy.io.savemat(f'./Result/f4/FEM2Transformer/'+file_list[j]+'.mat', {'result': result_combined})
